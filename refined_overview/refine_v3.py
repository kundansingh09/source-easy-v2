...
CACHE_DIR           = "cache_refine_mini"

# Per-field content budgets (characters).
# Two-tier routing: if about >= ABOUT_RICH_THRESHOLD chars (already SEMICON-filtered,
# high signal), website_scrape is skipped entirely — about alone is the better source.
# website_scrape is only used when about is sparse, and capped tighter as it's noisy.
ABOUT_RICH_THRESHOLD   = 400   # skip website_scrape above this
ABOUT_MAX_CHARS        = 2000
WEBSITE_MAX_CHARS      = 1500  # only used when about < ABOUT_RICH_THRESHOLD
...
def build_inputs(records: list[dict]) -> list[CompanyInput]:
    ...
    for i, r in enumerate(records):
        ...
        # Two-tier routing: rich about → skip website_scrape (higher quality source)
        ws_raw = (r.get("website_scrape") or "").strip()
        ws = "" if len(about) >= ABOUT_RICH_THRESHOLD else ws_raw

        # Category names as a short context hint. IDs are not sent — they're
        # numeric keys the model can't interpret; names carry the real meaning.
...
def build_prompt(ci: CompanyInput) -> str:
...
def process(ci: CompanyInput, session: requests.Session, api_key: str,
    ...
    if os.path.exists(cpath):
        with open(cpath, encoding="utf-8") as f:
            return RefinedResult(**json.load(f))
